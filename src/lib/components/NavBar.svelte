		  ...
		  <div class="dropdown dropdown-end">
			...
			<ul
			  ...
			  <li><form method="POST" action="/logout" use:enhance>
				<button class="p-0 text-secondary font-normal no-underline hover:no-underline hover:text-error no-animation" type="submit">Logout</button>
			</form></li>
			</ul>
		  </div>
		  {:else}
		  <a href="/signin" class="btn btn-ghost text-secondary hover:btn-accent hover:text-accent font-satoshi font-bold p-4 h-20">
