...
export async function checkReservation() {
  ...
  for (let i = 0; i < records.length; i++) {
    if (
      now >= new Date(records[i].start).toISOString() &&
      now <= new Date(records[i].end).toISOString()
    ) {
      reservation = {
...
export async function getHallOfFame() {
  let records = await pb.collection("42_hall_of_fame").getFullList({
    sort: "-created",
    fields:
      "id,author,author_meta,title,description,tags,media,votes,saves,published,comments,created",
  });
  // for (let i = 0; i < records.length; i++) {
...
export async function searchHallOfFame(search) {
  const records = await pb.collection("42_hall_of_fame").getFullList({
    ...
    sort: "-created",
    fields:
      "id,author,title,description,tags,media,votes,saves,published,comments,created",
  });
  return records;
}
